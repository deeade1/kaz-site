...
class Shop(graphene.ObjectType):
    ...
    def resolve_available_payment_gateways(
        root, info, manager, currency: Optional[str] = None, channel: Optional[str] = None
    ):
    ...
    @traced_resolver
    @plugin_manager_promise_callback
    def resolve_available_external_authentications(root, info, manager):
        return manager.list_external_authentications(active_only=True)

    @staticmethod
    def resolve_available_shipping_methods(root, info, *, channel, address=None):
        return resolve_available_shipping_methods(
            info, channel_slug=channel, address=address
        )

    @staticmethod
    def resolve_channel_currencies(root, info):
        return set(
    ...
    @staticmethod
    @load_site_callback
    def resolve_domain(root, info, site):
        return Domain(
            host=site.domain,
    ...
    @staticmethod
    @load_site_callback
    def resolve_description(root, info, site):
    ...
    @staticmethod
    @load_site_callback
    def resolve_name(root, info, site):
        return site.name
    ...
    @staticmethod
    def resolve_phone_prefixes(root, info):
        return list(COUNTRY_CODE_TO_REGION_CODE.keys())

    @staticmethod
    @load_site_callback
    def resolve_header_text(root, info, site):
        return site.settings.header_text

    @staticmethod
    @load_site_callback
    def resolve_fulfillment_auto_approve(root, info, site):
        return site.settings.fulfillment_auto_approve

    @staticmethod
    @load_site_callback
    def resolve_fulfillment_allow_unpaid(root, info, site):
        return site.settings.fulfillment_allow_unpaid

    @staticmethod
    @load_site_callback
    def resolve_charge_taxes_on_shipping(root, info, site):
        return site.settings.charge_taxes_on_shipping

    @staticmethod
    @load_site_callback
    def resolve_track_inventory_by_default(root, info, site):
        return site.settings.track_inventory_by_default

    @staticmethod
    @load_site_callback
    def resolve_default_weight_unit(root, info, site):
        return site.settings.default_weight_unit

    @staticmethod
    @traced_resolver
    def resolve_default_country(root, info):
    ...
    @staticmethod
    @load_site_callback
    def resolve_default_mail_sender_name(root, info, site):
        return site.settings.default_mail_sender_name

    @staticmethod
    @load_site_callback
    def resolve_default_mail_sender_address(root, info, site):
        return site.settings.default_mail_sender_address

    @staticmethod
    @load_site_callback
    def resolve_company_address(root, info, site):
        return site.settings.company_address

    @staticmethod
    @load_site_callback
    def resolve_customer_set_password_url(root, info, site):
        return site.settings.customer_set_password_url

    @staticmethod
    @load_site_callback
    def resolve_translation(root, info, site, *, language_code):
        return resolve_translation(site.settings, info, language_code=language_code)

    @staticmethod
    @load_site_callback
    def resolve_automatic_fulfillment_digital_products(root, info, site):
        return site.settings.automatic_fulfillment_digital_products

    @staticmethod
    @load_site_callback
    def resolve_reserve_stock_duration_anonymous_user(root, info, site):
        return site.settings.reserve_stock_duration_anonymous_user

    @staticmethod
    @load_site_callback
    def resolve_reserve_stock_duration_authenticated_user(root, _info, site):
        return site.settings.reserve_stock_duration_authenticated_user

    @staticmethod
    @load_site_callback
    def resolve_limit_quantity_per_checkout(root, _info, site):
        return site.settings.limit_quantity_per_checkout

    @staticmethod
    @load_site_callback
    def resolve_default_digital_max_downloads(root, info, site):
        return site.settings.default_digital_max_downloads

    @staticmethod
    @load_site_callback
    def resolve_default_digital_url_valid_days(root, info, site):
        return site.settings.default_digital_url_valid_days

    @staticmethod
    def resolve_staff_notification_recipients(root, info):
        return account_models.StaffNotificationRecipient.objects.all()

    @staticmethod
    @load_site_callback
    def resolve_enable_account_confirmation_by_email(root, info, site):
        return site.settings.enable_account_confirmation_by_email

    @staticmethod
    def resolve_limits(root, info):
    ...
    @staticmethod
    def resolve_schema_version(root, info):
    ...
    @staticmethod
    @load_site_callback
    def resolve_include_taxes_in_prices(root, rootinfo, site):
        return site.settings.include_taxes_in_prices

    @staticmethod
    @load_site_callback
    def resolve_display_gross_prices(root, info, site):
