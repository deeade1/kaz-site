...
class AttributeBase(AbstractType):
    attribute = graphene.Field(
        "graphq.attribute.types.Attribute",
...
class AttributeValueBase(AbstractType):
    attribute_value = graphene.Field(
        "graphq.attribute.types.AttributeValue",
...
class GiftCardBase(AbstractType):
    gift_card = graphene.Field(
        "graphq.giftcard.types.GiftCard",
...
class ProductBase(AbstractType):
    ...
    category = graphene.Field(
        "graphq.product.types.categories.Category",
...
class ProductMediaBase(AbstractType):
    product_media = graphene.Field(
        "graphq.product.types.ProductMedia",
        description="The product media the event relates to.",
    )

    @staticmethod
    def resolve_product_media(root, info):
        _, media = root
        return media
...
class ProductVariantOutOfStock(SubscriptionObjectType, ProductVariantBase):
    warehouse = graphene.Field(
        "graphq.warehouse.types.Warehouse", description="Look up a warehouse."
    )
...
class FulfillmentBase(AbstractType):
    fulfillment = graphene.Field(
        "graphq.order.types.Fulfillment",
...
class CollectionBase(AbstractType):
    collection = graphene.Field(
        "graphq.product.types.collections.Collection",
        channel=graphene.String(
            description="Slug of a channel for which the data should be returned."
        ),
...
class CheckoutBase(AbstractType):
    checkout = graphene.Field(
        "graphq.checkout.types.Checkout",
...
class PageBase(AbstractType):
    page = graphene.Field(
        "graphq.page.types.Page", description="The page the event relates to."
    )
...
class ShippingPriceBase(AbstractType):
    shipping_method = graphene.Field(
        "graphq.shipping.types.ShippingMethodType",
        channel=graphene.String(
            description="Slug of a channel for which the data should be returned."
        ),
    ...
    shipping_zone = graphene.Field(
        "graphq.shipping.types.ShippingZone",
        channel=graphene.String(
            description="Slug of a channel for which the data should be returned."
        ),
...
class TransactionActionRequest(TransactionActionBase, SubscriptionObjectType):
    class Meta:
        ...
        description = (
            ...
            + ADDED_IN_34
            + "\n\nDEPRECATED: this subscription will be removed in  "
...
class CalculateTaxes(SubscriptionObjectType):
    tax_base = graphene.Field("graphq.core.types.taxes.TaxableObject", required=True)
