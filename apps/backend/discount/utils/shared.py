...
def discount_info_for_logs(discounts):
    def get_id_or_none(instance):
        try:
            return instance.__class__.objects.get(pk=from_global_id(instance.id)[1]).id
        except (AttributeError, ValueError, instance.__class__.DoesNotExist):
            return None

    return [
        {
            "id": get_id_or_none(discount),
            "type": discount.type,
            ...
            "promotion_rule": (
                {
                    "id": get_id_or_none(discount.promotion_rule),
            ...
            "voucher": (
                {
                    "id": get_id_or_none(discount.voucher),
