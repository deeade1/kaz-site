...
from graphq.discount.utils import get_variants_for_catalogue_predicate
...
def clear_promotion_rule_variants_task():
    ...
    promotions = Promotion.objects.using().expired()
    rules = PromotionRule.objects.using().filter(
        Exists(promotions.filter(id=OuterRef("promotion_id")))
    )
