...
class OrderDiscount(BaseDiscount):
    ...
    class Meta:
        indexes = [
            # Index for querying the `promotion_rule` field efficiently
            BTreeIndex(
                fields=["promotion_rule"], name="orderdiscount_promotion_rule_idx"
            ),
            # GIN index for searching the `name` and `translated_name` fields
            GinIndex(
                fields=["name", "translated_name"],
                opclasses=[
                    "gin_trgm_ops",
                    "gin_trgm_ops",
                ],  # Ensure opclasses match the fields
                name="orderdiscount_translated_idx",
            ),
            # Index for querying the `voucher_code` field efficiently
            BTreeIndex(fields=["voucher_code"], name="orderdiscount_voucher_code_idx"),
        ]
...
class OrderLineDiscount(BaseDiscount):
    ...
    class Meta:
        indexes = [
            ...
            BTreeIndex(fields=["voucher_code"], name="orderlinedisc_voucher_code_idx"),
        ]
        ordering = ("created_at", "id")
        constraints = [
...
class CheckoutDiscount(BaseDiscount):
    ...
    class Meta:
        ...
        indexes = [
            # BTree index for efficient filtering on `promotion_rule`
            BTreeIndex(fields=["promotion_rule"], name="checkoutdiscount_rule_idx"),
            # GIN index for full-text search on `name` and `translated_name` using trigram operators
            GinIndex(
                fields=["name", "translated_name"],
                opclasses=[
                    "gin_trgm_ops",
                    "gin_trgm_ops",
                ],  # Ensure one operator class per field
                name="checkoutdisc_trans_gin_idx",
            ),
            # BTree index for filtering on `voucher_code`
            BTreeIndex(fields=["voucher_code"], name="checkoutdiscount_voucher_idx"),
        ]
...
class CheckoutLineDiscount(BaseDiscount):
    ...
    class Meta:
        indexes = [
            # Index for efficiently querying the `promotion_rule` field
            BTreeIndex(
                fields=["promotion_rule"], name="checklinedisc_promotion_rule_idx"
            ),
            # Index for efficiently querying the `voucher_code` field
            BTreeIndex(fields=["voucher_code"], name="checklinedisc_voucher_code_idx"),
        ]
...
class PromotionEvent(models.Model):
    ...
    # app = models.ForeignKey(App, blank=True, null=True, related_name="promotion_events", on_delete=models.SET_NULL,)
    promotion = models.ForeignKey(
