...
from accounts import events as account_events
...
def event_transaction_cancel_requested(
    order_id: "UUID", reference: str, user: Optional[User]
):
    return OrderEvent.objects.create(
        order_id=order_id,
...
def draft_order_created_event(
    *, order: Order, user: Optional[User], 
) -> OrderEvent:
    return OrderEvent.objects.create(
...
def order_created_event(
    *, order: Order, user: Optional[User], from_draft=False
) -> OrderEvent:
    if from_draft:
...
def order_confirmed_event(
    *, order: Order, user: Optional[User]
    ...
    return OrderEvent.objects.create(
        order=order, type=OrderEvents.CONFIRMED, user=user
    )
...
def payment_voided_event(
    *, order: Order, user: Optional[User], payment: Payment
) -> OrderEvent:
    return OrderEvent.objects.create(
        order=order,
...
def transaction_mark_order_as_paid_failed_event(
    order: Order, user: Optional[User],  message: str
):
    parameters = {"message": message}
