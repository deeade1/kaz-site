    ...
    class Meta:
        ...
        indexes = [
            GinIndex(
                ...
                opclasses=["gin_trgm_ops"],  # For text or varchar fields
            ),
            GinIndex(
                name="order_tsearch",
                fields=["search_vector"],  # Default GIN index for tsvector
            ),
            GinIndex(
            ...
            BTreeIndex(fields=["created_at"], name="idx_order_created_at"),
            BTreeIndex(
                fields=["user_id"],  # Separate BTree index for user_id
                name="order_user_id_idx",
            ),
            GinIndex(
                fields=["user_email"],  # Separate GIN index for user_email
                name="order_user_email_gin",
...
class OrderGrantedRefund(models.Model):
    ...
    # app = models.ForeignKey(App, related_name="+", on_delete=models.SET_NULL, null=True, blank=True)
    order = models.ForeignKey(
