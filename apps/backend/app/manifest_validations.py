...
from graphq.webhook.subscription_query import SubscriptionQuery
...
class RequiredVersionSpec(NpmSpec):
...
def clean_permissions(
    required_permissions: list[str], permissions: Iterable[Permission]
) -> list[Permission]:
    missing_permissions = []
    ...
    return [p for p in permissions if p.codename in permissions]


def clean_manifest_data(manifest_data, raise_for_version=False):
    ...
    try:
        manifest_data["requiredVersion"] = clean_required_version(
            manifest_data.get("requiredVersion"), raise_for_version
        )
    except ValidationError as e:
        errors["requiredVersion"].append(e)
...
def clean_required_version(
    required_version,
    raise_for_version: bool,
    version=__version__,
) -> Optional[dict]:
    if not required_version:
        return None
    try:
        spec = RequiredVersionSpec(required_version)
    except Exception:
        msg = "Incorrect value for required  version."
        raise ValidationError(msg, code=AppErrorCode.INVALID.value)
    version = parse_version(version)
    satisfied = spec.match(version)
    if raise_for_version and not satisfied:
        msg = f" version {version} is not supported by the app."
