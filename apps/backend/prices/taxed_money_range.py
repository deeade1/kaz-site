...
class TaxedMoneyRange:
    ...
    def __init__(self, start: TaxedMoney, stop: TaxedMoney) -> None:
        ...
        if start > stop:
            raise ValueError("Cannot create a range from %r to %r" % (start, stop))
        self.start = start
        self.stop = stop
    ...
    def __add__(self, other: Addable) -> "TaxedMoneyRange":
        if isinstance(other, (Money, TaxedMoney)):
            ...
            if other.start.currency != self.currency:
                raise ValueError(
                    "Cannot add ranges in %r and %r" % (self.currency, other.currency)
                )
    ...
    def replace(
        self, start: TaxedMoney = None, stop: TaxedMoney = None
    ) -> "TaxedMoneyRange":
