...
def customer_account_activated_event(
    *, staff_user: Optional[User], account_id: int
...
def customer_deleted_event(
    *, staff_user: Optional[User], deleted_count: int = 1
) -> CustomerEvent:
    return CustomerEvent.objects.create(
        user=staff_user,
        order=None,
...
def assigned_email_to_a_customer_event(
    *, staff_user: Optional[User], new_email: str
) -> CustomerEvent:
    return CustomerEvent.objects.create(
        user=staff_user,
        order=None,
...
def assigned_name_to_a_customer_event(
    *, staff_user: Optional[User], new_name: str
) -> CustomerEvent:
    return CustomerEvent.objects.create(
        user=staff_user,
        order=None,
